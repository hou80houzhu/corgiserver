...
var corgi = function () {
    this.basePath = packet.path(__dirname).parent().getPath();
...
corgi.parseQueryString = function (str) {
    ...
    if (str) {
        str.split("&").forEach(function (a) {
            ...
            if (!r[b[0]]) {
                r[b[0]] = b[1];
            } else if (packet.is.isArray(r[b[0]])) {
                r[b[0]].push(b[1]);
            } else {
                r[b[0]] = [r[b[0]]];
            }
...
corgi.prototype.startup = function () {
    packet.file(this.serverConfigPath).read().scope(this).then(function (data) {
        this.serverConfig = new serverConfig(JSON.parse(data), this.basePath);
    }).then(function () {
        return packet.file(this.webConfigPath).read();
    }).then(function (data) {
...
corgi.prototype.initProjects = function (code) {
    var path = this.basePath + "webapps", ths = this, queue = packet.queue(), ps = packet.promise();
    ...
    packet.file(path).getSubPaths().done(function (data) {
        ...
        data.file.forEach(function (a) {
            if (a.indexOf(".json") !== -1) {
                queue.add(function (a, b) {
                    var tss = this;
                    packet.file(b).read().done(function (data) {
...
corgi.prototype.initNspContainer = function () {
    ...
    packet.setTemplateGlobalMacro("include", function (attrs, render) {
        var c = global.CorgiServer.getCspContent(this.basePath + attrs.path), t = "";
        if (!c) {
            c = "";
        }
        try {
            var temp = packet.template(c);
    ...
    packet.setTemplateGlobalMacro("error", function (attr, render) {
        var a = "";
        try {
...
corgi.prototype.running = function (data, req, res) {
    var resp = response(), reqt = request(req, packet.extend(data, {method: req.method.toLowerCase(), url: req.url, rawHeaders: req.rawHeaders}));
...
corgi.prototype.createProject = function (name, path) {
    ...
    var p = this.basePath;
    packet.file(p + "webapps/" + name + ".json").write(JSON.stringify({
        name: name,
        path: path
    }, null, 4)).then(function () {
        return packet.file(path + "WEBINF/web.json").write(JSON.stringify({
        ...
        return packet.file(path + "WEBINF/src/controller.js").write(
                "/*\n" +
                " * @packet controller;\n" +
                " */\n" +
                "Module({\n" +
        ...
        return packet.file(path + "index.html").write(
                "<!DOCTYPE html>\n" +
                "<html>\n" +
                "    <head>\n" +
                "        <title>index</title>\n" +
        ...
        return packet.file(path + "index.csp").write(
                "<!DOCTYPE html>\n" +
                "<html>\n" +
                "    <head>\n" +
...
global.CorgiServer = {
    ...
    setTemplateMacro: function (key, fn) {
        packet.setTemplateGlobalMacro(key, fn);
    }
};

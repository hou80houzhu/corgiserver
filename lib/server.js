...
corgi.prototype.startup = function () {
    bright.file(this.serverConfigPath).read().scope(this).then(function (data) {
...
corgi.prototype.running = function (data, req, res) {
    ...
    var resp = response(), reqt = request(req, bright.extend(data, {method: req.method.toLowerCase(), url: req.url + t, rawHeaders: req.rawHeaders}));
...
corgi.prototype.createProject = function (name, path) {
    ...
    var p = this.basePath;
    bright.file(p + "webapps/" + name + ".json").write(JSON.stringify({
        name: name,
        path: path

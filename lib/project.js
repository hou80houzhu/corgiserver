...
var project = function (path, name, isouter) {
    ...
    this._services = [];
    this._filters = [];
    this._packet = brooder.packet(this._path + "WEBINF/packets/");
    this.config = null;
    this._scope = {};
    this._session = {};
};
project.prototype.packetInit = function (fn) {
    var ths = this;
    brooder.file(this._path + "WEBINF/packets/").getAllSubFilesPathWithSuffix("js").done(function (t) {
        var n = "";
        if (t.length > 0) {
            var que = brooder.queue(), n = "";
...
project.prototype.run = function (code, fn) {
    ...
    this.sessionWatcher();
    brooder.file(this._path + "WEBINF/web.json").read().done(function (data) {
        try {
...
project.prototype.getModule = function (packetName, option) {
    ...
    if (c) {
        ...
        throw Error("[brooder] can not find the module of " + packetName);
    }
};
project.prototype.doFilters = function (request, response, fn) {
    ...
    if (this.config._data.filter.length > 0) {
...
project.prototype.doServices = function (fn) {
    var ths = this;
    if (this.config._data.service.length > 0) {
        var queue = brooder.queue();

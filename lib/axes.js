...
axes.util = util;
axes.json = json;
axes.extend = axes.json.cover;
axes.serialize = serialize;
axes.nfn = function () {};
...
file.prototype.remove = function () {
    var ps = axes.promise(), path = this.path;
...
file.prototype.getSubFilesPath = function () {
    var tmpPath = this.path, ps = axes.promise(), r = [];
    try {
...
file.prototype.getSubPaths = function () {
    var tmpPath = this.path, ps = axes.promise(), f = [], fd = [];
    try {
        var stats = fs.statSync(tmpPath);
        if (stats.isDirectory()) {
            fs.readdirSync(tmpPath).forEach(function (item) {
    ...
    if (parent && parent.prototype[propName]) {
        ...
        if (axes.is.isFunction(b)) {
            ...
            for (var i in self.prototype) {
                if (axes.is.isFunction(self.prototype[i])) {
                    if (i !== propName) {
                        pp.push(i);
                        (function (mp) {
                            _a[mp] = function () {
...
factory.prototype.def = function (obj) {
    ...
    axes.is.isString(obj.extend) && (array = [obj.extend]);
    a._parent = array[0];
...
factory.prototype.invoke = function (clazzName, methodName, scope) {
    var a = null;
    if (axes.is.isString(clazzName)) {
        var j = this.mapping[clazzName];
        j && (a = new j());
    } else if (axes.is.isObject(clazzName)) {
        a = clazzName;
    }
    if (a && a[methodName]) {
        if (axes.is.isFunction(a[methodName]) && axes.is.isObject(scope)) {
...
factory.prototype.proxy = function (object, part, fn) {//fn(method)
    if (arguments.length > 1) {
        ...
        if (axes.is.isObject(object)) {
            if (axes.is.isArray(part)) {
                some = part;
                proxy = fn;
            } else if (axes.is.isFunction(part)) {
                proxy = part;
            }
            var a = new this.mapping[object.__adapt__._type]();
            for (var i in object) {
                if (axes.is.isFunction(object[i])) {
...
packet.isProjectPathTag = /["\']@\/[A-Za-z0-9_-]+["\']/g;
...
packet.replacePacketNames = function (info, code, projectPath) {
    return code.replace(packet.isProjectPathTag, function (str) {
        return (str[0] === "'" ? "'" : "\"") + projectPath + str.substring(2);
...
packet.run = function (packetName) {
    ...
    if (re.length === info.length) {
        for (var i = 0; i < info.length; i++) {
            ...
            xp = packet.deleteR(xp) + "axes.is.isEmptyObject(module.exports)?(module.exports=exports):'';";
            if (d.usestrict === "true") {
                d.usestrict = true;
...
axes.packet = function (path, basepath) {

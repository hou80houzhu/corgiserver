...
Module({
    ...
    getModule: function (type, option) {
        return packetLoader.get(type, axes.extend({
...
Module({
    ...
    render: function (goon) {
        ...
        var t = "";
        try {
            t = axes.template(this.option.template).render(this.option.data);
        } catch (e) {
...
Module({
    ...
    render: function (goon) {
        ...
        if (content) {
            ...
            try {
                var temp = axes.template(content);
...
Module({
    ...
    _startview: function (done) {
        ...
        axes.file(tempPath).getAllSubFilesPathWithSuffix(suffix).done(function (p) {
            if (p.length > 0) {
                var n = "", que = axes.queue();
                ...
                p.forEach(function (path) {
                    que.add(function (a, b) {
                        var q = this;
                        axes.file(b).read().done(function (content) {
...
Module({
    ...
    with : function (req) {
        axes.extend(this._data, req._data);
        return this;
    },

...
Module({
    ...
    getModule: function (type, option) {
        return packetLoader.get(type, brooder.extend({
...
Module({
    ...
    getTemplateView: function (template, data) {
        var a = packetLoader.get("templateview", {request: this.request, response: this.response, data: data, template: project.getAttr("template")[template]});
        return a;
    },
...
Module({
    ...
    render: function (goon) {
        ...
        var t = "";
        try {
            t = brooder.template(this.option.template).render(this.option.data);
        } catch (e) {
...
Module({
    ...
    render: function (goon) {
        ...
        var content = global.CorgiServer.getNspContent(path);
        if (content) {
            response.setStatusCode(200);
            response.setContentType("text/html");
            try {
                var temp = brooder.template(content);
...
Module({
    ...
    start: function (done) {
        ...
        packetLoader.each(function (mod) {
            if (mod.typeOf("controller")) {
                a[mod.getFields()["path"]] = {name: mod.type()};
            }
        });
        packetLoader.each(function (mod) {
            if (mod.typeOf("table")) {
        ...
        project.setAttr("controllerMapping", a);
    ...
    _startview: function (done) {
        var ths = this, tempPath = project.getProjectPath() + this.option.view.path + "/", suffix = this.option.view.suffix;
        brooder.file(tempPath).getAllSubFilesPathWithSuffix(suffix).done(function (p) {
            if (p.length > 0) {
                var n = "", que = brooder.queue();
                ...
                p.forEach(function (path) {
                    que.add(function (a, b) {
                        var q = this;
                        brooder.file(b).read().done(function (content) {
    ...
    _parseTemplate: function (template) {
        if (!project.hasAttr("template")) {
            project.setAttr("template", {});
        }
        template = template.replace(/>[\s]+</g, "><").replace(/\r\n/g, "").replace(/\r/g, "").replace(/\n/g, "");
        var _a = template.split(/\<\!\-\-\[@[\s\S]*?;\]\-\-\>/);
        if (_a.length > 1) {
            ...
            for (var i = 0; i < _b.length; i++) {
                ...
                if (temp !== "") {
                    ...
                    for (var j = 0; j < b.length; j++) {
                        ...
                        project.getAttr("template")[pn + "." + c] = p[j + 1].replace(/\<\!\-\-[\s\S]*?\-\-\>/g, "");
                    }
                }
            }
        }
...
Module({
    ...
    with : function (req) {
        brooder.extend(this._data, req._data);
        return this;
    },
...
Module({
    ...
    doFilter: function (data, next) {
        ...
        var b = a.substring(name.length + 2);
        var c = b.split("/");
        var action = c.splice(c.length - 1);
        var controller = "/" + c.join("/");
        var doit = false;
        if (!/^[\s\S]+\.[a-z]+$/.test(action)) {
            action = action.join("");
            if (action === "") {
                action = "index";
            }
            console.log("controller:[" + controller + "]   action:[" + action + "]");
            var pt = project.getAttr("controllerMapping")[controller];
            if (pt) {
                var packet = pt.name;
                var mod = packetLoader.get(packet);
                if (mod && mod.typeOf("controller")) {
                    ...
                    if (mod[action]) {
                        doit = true;
                        try {
                            mod[action](function (a) {
                                if (mod.dao) {
                                    mod.dao.privator("release");
                                }
                                next(a);
                            });
                        } catch (e) {
                            console.error(e.stack);
                            next(this.getModule("defaultPageView", {code: "500", data: e.stack}));
                        }
                    } else {
                        doit = true;

...
Module({
    ...
    getModule: function (type, option) {
        return packetLoader.get(type, packet.extend({
...
Module({
    ...
    render: function (goon) {
        ...
        var t = "";
        try {
            t = packet.template(this.option.template).render(this.option.data);
        } catch (e) {
...
Module({
    ...
    render: function (goon) {
        ...
        if (content) {
            ...
            try {
                var temp = packet.template(content);
...
Module({
    ...
    _startview: function (done) {
        ...
        packet.file(tempPath).getAllSubFilesPathWithSuffix(suffix).done(function (p) {
            if (p.length > 0) {
                var n = "", que = packet.queue();
                ...
                p.forEach(function (path) {
                    que.add(function (a, b) {
                        var q = this;
                        packet.file(b).read().done(function (content) {
...
Module({
    ...
    with : function (req) {
        packet.extend(this._data, req._data);
        return this;
    },

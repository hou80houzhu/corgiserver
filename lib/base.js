    ...
    render: function (goon) {
        ...
        if (path.indexOf("webapps" + Path.sep + project.getProjectName() + Path.sep + "WEBINF") === -1 && path.indexOf("webapps" + Path.sep + project.getProjectName() + Path.sep + "node_modules") === -1) {
            ...
            if (suffix === "csp") {
                ...
                var mime = CorgiServer.getWebConfig().mime[suffix] || "text/html";
                this.info = {
                    path: path,
                    suffix: suffix,
                    mime: mime
...
Module({
    ...
    render: function (goon) {
        ...
        var t = "";
        try {
            t = topolr.template(this.option.template).render(this.option.data);
        } catch (e) {
...
Module({
    ...
    render: function (goon) {
        this.option.response.pipe(request(this.option.url));
        this.done();
    }
});
Module({
    ...
    render: function (goon) {
        ...
        if (project.getProjectConfig().hasPage(this.option.code)) {
            ...
            path = global.CorgiServer.getWebConfig().page[this.option.code] || "404.csp";
        }
...
Module({
    ...
    _startview: function (done) {
        ...
        var p = topolr.file(tempPath).scan(function (path, isfile) {
            if (isfile && topolr.path(path).suffixWith(suffix)) {
                return path;
            }
        });
        if (p.length > 0) {
            var n = "", que = topolr.queue();
            ...
            p.forEach(function (path) {
                que.add(function (a, b) {
                    var q = this;
                    topolr.file(b).read().done(function (content) {
                        try {
                            if (/\<\!\-\-\[@[\s\S]*?;\]\-\-\>/.test(content)) {
...
Module({
    ...
    transaction: function () {
        ...
        }).scope(this).complete(function (a, b) {
            this._transactioncount--;
            if (b) {

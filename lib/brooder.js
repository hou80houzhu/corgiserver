...
file.prototype.getSubPaths = function () {
    var tmpPath = this.path, ps = brooder.promise(), f = [], fd = [];
    try {
        var stats = fs.statSync(tmpPath);
        if (stats.isDirectory()) {
            fs.readdirSync(tmpPath).forEach(function (item) {
...
brooder.path = function (p, isfolder) {
    ...
    if (parent && parent.prototype[propName]) {
        ...
        if (brooder.is.isFunction(b)) {
            ...
            for (var i in self.prototype) {
                if (brooder.is.isFunction(self.prototype[i])) {
                    if (i !== propName) {
                        pp.push(i);
                        (function (mp) {
                            _a[mp] = function () {
...
factory.prototype.def = function (obj) {
    ...
    brooder.is.isString(obj.extend) && (array = [obj.extend]);
    a._parent = array[0];
    ...
    for (var i in obj) {
        if (factory.d.test(i)) {
            ...
            if (brooder.is.isFunction(obj[i])) {
                if (factory.e.test(i)) {
                    a._private[i] = obj[i];
                } else {
                    prpt[i] = obj[i];
...
factory.prototype.invoke = function (clazzName, methodName, scope) {
    var a = null;
    if (brooder.is.isString(clazzName)) {
        var j = this.mapping[clazzName];
        j && (a = new j());
    } else if (brooder.is.isObject(clazzName)) {
        a = clazzName;
    }
    if (a && a[methodName]) {
        if (brooder.is.isFunction(a[methodName]) && brooder.is.isObject(scope)) {
...
factory.prototype.proxy = function (object, part, fn) {//fn(method)
    if (arguments.length > 1) {
        ...
        if (brooder.is.isObject(object)) {
            if (brooder.is.isArray(part)) {
                some = part;
                proxy = fn;
            } else if (brooder.is.isFunction(part)) {
                proxy = part;
            }
            var a = new this.mapping[object.__adapt__._type]();
            for (var i in object) {
                if (brooder.is.isFunction(object[i])) {
...
packet.run = function (packetName) {
    ...
    if (re.length === info.length) {
        for (var i = 0; i < info.length; i++) {
            ...
            xp = packet.deleteR(xp) + "brooder.is.isEmptyObject(module.exports)?(module.exports=exports):'';";
            if (d.usestrict === "true") {
                d.usestrict = true;
...
brooder.packet = function (basepath) {
